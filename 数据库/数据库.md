...
2. xml
   ...
   1. 配置 xml
      1. 路径：通常在 resource/Mapper/下
      2. yml：
      ```yml
      mybatis:
        mapper-locations: classpath:Mapper/*.xml # 若 XML 在 resources/Mapper 目录下
      ```
   2. xml 写法
      ...
      <mapper namespace="com.example.justtest.Mapper.UserMapper">

      <!-- id 必须与接口中的方法名（GetByPhone）一致 -->
      <!-- resultType 指定返回值类型（User 实体类的全类名） -->
      <select id="register" resultType="com.example.justtest.Entity.User">
         <!-- SQL 语句：根据 phone 查询 password -->
         insert into tb_user ( password, phone)
