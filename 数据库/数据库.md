- [redis](#redis)
  - [基础语法](#基础语法)
  - [redis 事务与 lua 脚本](#redis-事务与-lua-脚本)
  - [秒杀](#秒杀)
  - [如何保证 redis 和数据库里的数据一致？](#如何保证-redis-和数据库里的数据一致)
  - [redis 持久化](#redis-持久化)
    - [RDB](#rdb)
    - [AOF（Append Only File）](#aofappend-only-file)
  - [redis 过期策略](#redis-过期策略)
  - [redis 数据淘汰策略](#redis-数据淘汰策略)
- [MySQL](#mysql)
  - [底层结构](#底层结构)
    - [对于单张页](#对于单张页)
    - [对于多张表](#对于多张表)
...
1.  数据查询
    ...
    - 示例
      ...
      | 101 | 1 | 150 |
            ...
            不要GROUP BY user_id有什么影响？
            答：不要会导致id和order_count统计的是全部的。并且user_id会是随机一条，或者报错
